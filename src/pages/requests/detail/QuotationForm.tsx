...
export default function QuotationForm({ onSubmit, onCancel }: Props) {
  ...
  function validate() {
    ...
    if (!form.supplier.trim()) e.supplier = 'Campo obrigatório'
    if (!form.price || Number(form.price) <= 0) e.price = 'Informe um valor válido'
    if (!form.deliveryDays || Number(form.deliveryDays) <= 0) e.deliveryDays = 'Informe o prazo'
    return e
  }
  ...
  function handleSubmit(e: FormEvent) {
    ...
    // Converte os valores de string para number antes de enviar
    onSubmit({
